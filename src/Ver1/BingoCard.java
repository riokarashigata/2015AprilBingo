...
public class BingoCard 
{
    ...
    public static void main(String[] args)
    {
        ...
        bingoCardPrinter(bingoCardCreator());
        System.out.print("\n");
        
        int players = 5;
        
        int [][][] bingoCards = bingoCardsCreator(players);
        //bingoCardsPrinter_vertical(bingoCards);  
        bingoCardPrinter_horizontal(bingoCards);
        
    }
    
    /*指定された枚数だけビンゴカードを作成する */
    public static int[][][] bingoCardsCreator(int players)
    {
        int[][][] bingoCards = new int[players][5][5];
        int bingoNumber = 0;
    ...
    public static void bingoCardPrinter (int[][] bingoCard)
    {
        ...
        for(int column=0; column<bingoCard.length; column++)
        {
            ...
            {
                int bingoNumber = bingoCard[row][column];
                /* 数字を右揃えにする */
                if(bingoNumber < 10)
                {
                    /* 中央値はスペースを表示する */
            ...
            for(int player=0; player<bingoCards.length; player++)
            {
                for(int row=0; row<bingoCards[0].length; row++)
                {
                    ...
                    if(bingoNumber < 10)
                    {
                        ...
                        System.out.print(bingoCards[player][row][column]);
                    }
                    /* 各列をパイプ（｜）で区切る */
                    if(row==4)
                    {
